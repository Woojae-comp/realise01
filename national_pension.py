...
def read_pensiondata():
    dtype_spec = {
        '업종코드': 'str',  # 업종코드 열을 문자열로 명시적으로 지정
        '금액': 'float64',
        '가입자수': 'int64',
        # 필요한 다른 열의 dtype도 추가로 지정할 수 있습니다.
    }
    filepath = "https://drive.google.com/uc?id=1SXQV4Citkyp6ksqZv92E7-DYYzjWzJff"
    df = pd.read_csv(filepath, encoding='cp949', dtype=dtype_spec, low_memory=False)
    df.columns = [
